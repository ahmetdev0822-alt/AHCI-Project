...
Student Management screen – Admin only. Full CRUD with table and modal form.
"""
...
class StudentsScreen(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Filter: Class
        ctk.CTkLabel(toolbar, text="Class:",
                     font=(Fonts.FAMILY, Fonts.SIZE_SM),
                     text_color=Colors.TEXT_SECONDARY).pack(side="left", padx=(20, 4))
        ...
        for label, val, color, bg in [
            ("Active", str(sum(1 for s in self._state.students if s['status']=="Active")), Colors.SUCCESS, Colors.SUCCESS_BG),
            ("Male",   str(sum(1 for s in self._state.students if s['gender']=="M")),     Colors.INFO,    Colors.INFO_BG),
            ("Female", str(sum(1 for s in self._state.students if s['gender']=="F")),     Colors.ACCENT,  "#FDF8E7"),
        ]:
        ...
        columns = [
            ...
            {"key": "name",        "label": "Student Name",  "width": 180, "align": "w"},
            {"key": "class",       "label": "Class",         "width": 100, "align": "w"},
            {"key": "roll_no",     "label": "Roll #",        "width": 60,  "align": "center"},
            {"key": "gender",      "label": "Gender",        "width": 70,  "align": "center"},
            {"key": "parent_name", "label": "Parent Name",   "width": 160, "align": "w"},
    ...
    def _apply_filter(self, choice=None):
        choice = self._class_filter.get()
        if choice == "All":
            filtered = self._state.students
        else:
            filtered = [s for s in self._state.students if s["class"] == choice]
...
class StudentForm(ctk.CTkToplevel):
    """Modal form to add/edit a student."""
    ...
    def __init__(self, parent, state, data: dict | None, on_save):
        ...
        title = "Add New Student" if self._is_new else "Edit Student"
        self.title(title)
    ...
    def _build(self, title):
        ...
        name_row_entries = row_2([
            ("Class (e.g. Class 8-A)", d.get("class", "")),
            ("Roll Number",            d.get("roll_no", "")),
        ])
        self._class_entry, self._roll_entry = name_row_entries

        # Class dropdown - better UX
        label("Class *")
        ...
        self._roll_entry2 = roll_gender[0]
        label("Gender")
        ...
        ctk.CTkButton(
            btn_row, text="Save Student", width=140, height=36, corner_radius=8,
            fg_color=Colors.PRIMARY, hover_color=Colors.PRIMARY_DARK,
            text_color=Colors.TEXT_WHITE,
            command=self._save,
    ...
    def _save(self):
        ...
        data = {
            ...
            "roll_no":      self._roll_entry2.get().strip() or "—",
            "gender":       self._gender_var.get(),
            "parent_name":  self._parent.get().strip(),
            "phone":        self._phone.get().strip(),
            "email":        self._email.get().strip(),
            "admission_date": self._data.get("admission_date", "") if self._data else "",
...
class StudentDetailDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, state, data: dict):
        ...
        StatusBadge(header, data.get("status", "Active")).pack(side="right", padx=16, pady=16)
...
class ConfirmDialog(ctk.CTkToplevel):
    def __init__(self, parent, title, message, on_confirm):
        ...
        ctk.CTkLabel(self, text=message,
                     ...
                     wraplength=340, justify="center").pack(pady=(0, 16))

        btn_row = ctk.CTkFrame(self, fg_color="transparent")
        btn_row.pack()
        ctk.CTkButton(btn_row, text="Cancel", width=100, height=34, corner_radius=8,
                      fg_color=Colors.BG_INPUT, text_color=Colors.TEXT_SECONDARY,
                      command=self.destroy).pack(side="left", padx=8)
        ctk.CTkButton(btn_row, text="Yes, Delete", width=120, height=34, corner_radius=8,
